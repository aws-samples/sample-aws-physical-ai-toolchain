...
"""
Gamepad/keyboard teleoperation for UR3 (physical robot).

Opens a browser page that reads your gamepad via the HTML5 Gamepad API
or keyboard via WASD+IJKL, then sends velocity commands to the robot
over a persistent socket using URScript speedl.

Run this directly on EC2 or on the robot's local network (NOT through
an SSH tunnel -- use keyboard_teleop.py for that).

Usage:
    python3 examples/gamepad_teleop.py
    python3 examples/gamepad_teleop.py --record "pick up the cube"
    python3 examples/gamepad_teleop.py --port 8765
...
Controls (gamepad):
    ...
    Left Bumper       Precision mode (hold for 1/4 speed)
    Right Bumper      Fast mode (hold for 2x speed)
...
PHYSICAL_CAMERAS = {
    "wrist": os.environ.get("CAMERA_WRIST", "realsense"),
}
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .ep-item { display: flex; justify-content: space-between; align-items: center; padding: 4px 8px; border-bottom: 1px solid #334; font-size: 0.85em; }
  .ep-item:hover { background: #1a2a4e; }
  .ep-btn { padding: 3px 10px; border-radius: 4px; border: 1px solid #00d4ff; background: transparent; color: #00d4ff; cursor: pointer; font-size: 0.8em; }
  .ep-btn:hover { background: #00d4ff; color: #000; }
  .ep-btn.stop { border-color: #f44336; color: #f44336; }
  .ep-btn.stop:hover { background: #f44336; color: #fff; }
</style>
</head>
<body>
<h1>UR3 Teleop (Physical)</h1>
<div id="status" class="status warn">Press a gamepad button or any key to start...</div>
...
<div class="card controls">
  <b>Gamepad:</b> Left Stick X/Y &nbsp; Right Stick Z/Rot &nbsp; LT Open &nbsp; RT Close &nbsp; Y Home &nbsp; LB Slow &nbsp; RB Fast &nbsp; A Rec &nbsp; B E-stop<br>
  <b>Keyboard:</b> <b>WASD</b> X/Y &nbsp; <b>IK</b> Z up/down &nbsp; <b>JL</b> Rotate &nbsp; <b>Q</b> Open &nbsp; <b>E</b> Close &nbsp; <b>H</b> Home &nbsp; <b>Shift</b> Slow &nbsp; <b>Space</b> Fast &nbsp; <b>R</b> Rec &nbsp; <b>X</b> E-stop &nbsp; <b>Esc</b> Quit
</div>
...
function pollInput() {
  ...
  if (!gp && connected) {
    ...
    if (kbActive) {
      ...
      document.getElementById('status').textContent = 'Press a gamepad button or any key to start...';
    }
  }
...
function loadEpisodes() {
  fetch('/episodes', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: '{}'})
    .then(r => r.json())
    .then(eps => {
      const el = document.getElementById('ep-list');
      if (!eps.length) { el.innerHTML = '<div style="color:#888;padding:8px">No episodes</div>'; return; }
      el.innerHTML = eps.map(ep => {
        const name = ep.episode_id || '?';
        const esc = name.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
        const dur = (ep.duration_s || 0).toFixed(1);
...
def clamp_to_workspace(tcp, vx, vy, vz):
    ...
    if reach > MAX_REACH and reach > 0:
        if (x * vx + y * vy) / reach > 0:
            print(f"  CLAMP: reach {reach:.3f}m > {MAX_REACH}m")
            vx, vy = 0.0, 0.0
    if z <= MIN_Z and vz < 0:
        print(f"  CLAMP: z {z:.3f}m <= {MIN_Z}m")
        vz = 0.0
    if z >= MAX_Z and vz > 0:
        print(f"  CLAMP: z {z:.3f}m >= {MAX_Z}m")
        vz = 0.0
    return vx, vy, vz
...
def robot_loop():
    """Main robot control loop -- reads shared gamepad state, sends movel."""
    ...
    while not shared.quit:
        ...
        # Gripper -- skip speedl while gripper command is in flight
        ...
        if not gripper_active:
            vx = -ly * speed       # stick up (-1) -> robot +X
            vy = -lx * speed       # stick right (+1) -> robot -Y
            vz = -ry * speed       # right stick up (-1) -> robot +Z
            wrz = rx * ROT_SPEED
        ...
        # Record toggle (A / R, edge-triggered)
...
def main():
    ...
    if record_task and shared.robot:
        ...
        shared.recorder = EpisodeRecorder(
            ...
            cameras=PHYSICAL_CAMERAS,
        )

    server = http.server.ThreadingHTTPServer(("127.0.0.1", port), TeleopHandler)
