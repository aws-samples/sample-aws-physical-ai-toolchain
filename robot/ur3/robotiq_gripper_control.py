try:
    import rtde_control
except ImportError:
    pass

import socket
import time
from robot.ur3.robotiq_preamble import ROBOTIQ_PREAMBLE


class RobotiqGripper(object):
    """
    RobotiqGripper is a class for controlling a robotiq gripper using the
    ur_rtde robot interface or direct socket communication.

    Attributes:
        rtde_c (rtde_control.RTDEControlInterface): The interface to use for the communication
    """
    def __init__(self, rtde_c=None, robot_ip=None):
        """
        The constructor for RobotiqGripper class.

        Parameters:
           rtde_c (rtde_control.RTDEControlInterface): The interface to use for RTDE communication
           robot_ip (str): Robot IP address for socket communication
        """
        if rtde_c is not None:
            self.rtde_c = rtde_c
            self.control_method = "rtde"
        elif robot_ip is not None:
            self.robot_ip = robot_ip
            self.control_method = "socket"
        else:
            raise ValueError("Either rtde_c or robot_ip must be provided")

    def call(self, script_name, script_function):
        if self.control_method == "rtde":
            return self.rtde_c.sendCustomScriptFunction(
                "ROBOTIQ_" + script_name,
                ROBOTIQ_PREAMBLE + script_function
            )
        else:
            return self._send_script_via_socket(script_function)

    def _send_script_via_socket(self, script_function):
        """Send gripper URScript command via socket"""
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(10.0)
            s.connect((self.robot_ip, 30002))
            script = ROBOTIQ_PREAMBLE + "\n" + script_function
            indented = "\n".join("  " + line for line in script.split("\n"))
            program = f"def gripper_prog():\n{indented}\nend\n"
            s.send(program.encode('utf-8'))
            time.sleep(0.5)
            s.close()
            return True
        except Exception as e:
            print(f"Gripper socket command failed: {e}")
            return False

    def activate(self):
        """ 
        Activates the gripper. Currently the activation will take 5 seconds.
           
        Returns: 
            True if the command succeeded, otherwise it returns False
        """
        ret = self.call("ACTIVATE", "rq_activate()")
        time.sleep(5)  # HACK
        return ret

    def set_speed(self, speed):
        """ 
        Set the speed of the gripper. 
  
        Parameters: 
            speed (int): speed as a percentage [0-100]
          
        Returns: 
            True if the command succeeded, otherwise it returns False
        """
        return self.call("SET_SPEED", "rq_set_speed_norm(" + str(speed) + ")")

    def set_force(self, force):
        """ 
        Set the force of the gripper. 
  
        Parameters: 
            force (int): force as a percentage [0-100]
          
        Returns: 
            True if the command succeeded, otherwise it returns False
        """
        return self.call("SET_FORCE", "rq_set_force_norm(" + str(force) + ")")

    def move(self, pos_in_mm):
        """ 
        Move the gripper to a specified position in (mm).
  
        Parameters: 
            pos_in_mm (int): position in millimeters.
          
        Returns: 
            True if the command succeeded, otherwise it returns False
        """
        return self.call("MOVE", "rq_move_and_wait_mm(" + str(pos_in_mm) + ")")

    def open(self):
        """ 
        Open the gripper.
           
        Returns: 
            True if the command succeeded, otherwise it returns False
        """
        return self.call("OPEN", "rq_open_and_wait()")

    def close(self):
        """ 
        Close the gripper.
           
        Returns: 
            True if the command succeeded, otherwise it returns False
        """
        return self.call("CLOSE", "rq_close_and_wait()")
