...
| 11 | Cosmos Transfer 2.5 container in ECR | ✅ | Built from source (nvidia-cosmos/cosmos-transfer2.5) via CodeBuild. Runs as a NIM on EC2 p5 (NOT SageMaker — driver 470<580). `cosmos_setup.py` drives launch/status/generate/terminate; see docs/cosmos-deployment-guide.md. |
...
| 11b | Deploy Cosmos on EC2 Spot (p5.48xlarge) | ✅ | Spot p5 in us-east-2. H100 driver 580.159, fabricmanager required. Container running + HEALTHY. TRT engine built. |
