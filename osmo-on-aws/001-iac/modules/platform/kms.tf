...
# KMS key for OSMO data encryption
resource "aws_kms_key" "osmo" {
  description             = "KMS key for OSMO ${var.name_prefix} encryption"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Sid    = "Allow Auto Scaling to use the key for encrypted EBS volumes"
        Effect = "Allow"
        Principal = {
