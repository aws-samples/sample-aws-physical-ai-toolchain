#!/usr/bin/env python3
"""Negative plumbing fixture: deliberately inconsistent synthetic metrics.

Verifies mounted fixture weights, then reports a success rate that disagrees
with task counts. plumb_validate.py must reject it. This is not model evaluation.
"""
import hashlib
import json
import os
...
def main():
    ...
    # --- SABOTAGE VARIANT: success_rate deliberately mismatches weighted mean ---
    ...
    # Internally consistent per-task counts; only the aggregate is sabotaged.
    per_task = [
        {"task_id": 0, "successes": eval_trials, "trials": eval_trials, "rate": 1.0},
        {"task_id": 1, "successes": eval_trials // 2, "trials": eval_trials, "rate": (eval_trials // 2) / eval_trials},
    ]

    # Weighted mean from the selected trial count
    weighted_mean = sum(t["successes"] for t in per_task) / sum(t["trials"] for t in per_task)

    # SABOTAGE: success_rate disagrees with the weighted mean -- validator MUST catch this
    success_rate = 0.99  # deliberately inconsistent with the task counts
    print(f"[plumb_eval] SABOTAGE MODE: success_rate={success_rate} but "
