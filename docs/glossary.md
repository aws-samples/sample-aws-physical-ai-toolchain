...
| **GR00T** | NVIDIA's VLA foundation model (Generalist Robot 00 Technology). Pre-trained on diverse robot data. You fine-tune it on your specific robot + task. Version used in this toolchain: N1.6-3B (3 billion parameters). Supports robot arms and humanoids. | Default choice for this toolchain. Fine-tune with 50-100 teleoperated demonstrations. |
...
| **Cosmos Predict** | The generation capability within Cosmos. Given text, image, or video input, it generates future video frames that obey physics. In Cosmos 3, this is the primary mode of the Super and Nano models. Trained on 20M+ hours of physical world video. This is what Lab 3 uses (V2V generation from a reference clip + text prompt). | Core generation engine — produce synthetic training videos of robots/environments. |
...
| **Cosmos Guardrail** | Safety filtering component that screens both inputs and outputs for inappropriate or unsafe content during world generation. Requires accepting HuggingFace gated licenses (`nvidia/Cosmos-Guardrail1` and `nvidia/Cosmos-1.0-Guardrail`). vLLM-Omni downloads both at server startup. | Content safety — ensures generated training data doesn't contain problematic content. |
| **Cosmos 3** | The latest generation of NVIDIA's World Foundation Models (announced GTC Taipei 2025). A Mixture-of-Transformers (MoT) omnimodal architecture that combines vision reasoning and multimodal generation (text, video, images, sound, action) in a single model family. Supersedes the earlier separate Predict/Transfer/Reason models. | The current-gen platform. Lab 3 uses Cosmos 3 Super for synthetic demonstration generation. |
| **Cosmos 3 Super** | The 64B parameter variant of Cosmos 3. Highest quality generation. Requires 8x H100/H200 GPUs (640 GB+ total VRAM). Served via vLLM-Omni (`vllm/vllm-omni:cosmos3` container). | What Lab 3 deploys. Best quality, needs the most GPU. |
| **Cosmos 3 Nano** | The 16B parameter variant of Cosmos 3. Faster inference, fits on fewer GPUs (as few as 1-2 H100). Lower quality than Super but more accessible. Also used as the base for action-policy post-training. | Lighter alternative when Super's GPU requirements are too high. |
| **Cosmos Reason** | A vision-language model that *understands* video — answers questions about physics, spatial relationships, and what's happening in a scene. Does not generate video; it analyzes it. Used in the flywheel to evaluate whether generated videos are physically plausible (quality filter / rejection sampling). | Quality control — "does this generated video make physical sense?" |
